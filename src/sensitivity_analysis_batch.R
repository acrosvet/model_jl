...
incidence_plots <- function(type){
  ...
  p = ggplot(data = batch_incidence, aes(x = timestep, y = !!sym(type)), color = seq) +       
    geom_line(size = 0.05, alpha = 0.5) +
    geom_smooth(se = TRUE) +
    ggtitle(paste0("batch model (500 runs) ", {{type}}))
  
  ggsave(paste0("./export/plots/", "batch_inc_",type,".bmp"), plot = p)
...
batch_parms <- read_csv("./sensitivity_batch.csv") %>% mutate(seq = as.character(seq))
...
prcc_plots <- function(type){
  ...
  batch_res <- batch_incidence %>% select(seq, timestep, !!type)
  
  batch_parametrised <- left_join(batch_res, batch_parms, by = c("seq" = "seq"))
  
  prcc_frame <- batch_parametrised  %>% 
    ungroup() %>% 
    select(
      treatment_prob,
      density_calves,
  ...
  plot_prcc <- 
    ...
    theme_bw() +
    labs(
      title = paste0("PRCC (1000 batch simulations), daily % ", {{type}})) +
    xlab("PRCC (95% CI)") +
    ylab("Variable") +
  ...
  ggsave(paste0("./export/plots/batch", type, "_prcc",".bmp"), plot = plot_prcc)
}
