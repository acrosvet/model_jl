...
incidence_plots <- function(type){
  ...
  p = ggplot(data = spring_incidence, aes(x = timestep, y = !!sym(type)), color = seq) +       
    geom_line(size = 0.05, alpha = 0.5) +
    geom_smooth(se = TRUE) +
    ggtitle(paste0("Spring model (500 runs) ", {{type}}))
  
  ggsave(paste0("./export/plots/", "spring_inc_",type,".bmp"), plot = p)
...
spring_parms <- read_csv("./sensitivity_spring.csv") %>% mutate(seq = as.character(seq))
...
prcc_plots <- function(type){
  ...
  spring_res <- spring_incidence %>% select(seq, timestep, !!type)
  
  spring_parametrised <- left_join(spring_res, spring_parms, by = c("seq" = "seq"))
  
  prcc_frame <- spring_parametrised  %>% 
    ungroup() %>% 
    select(
      treatment_prob,
      density_calves,
  ...
  plot_prcc <- 
    ...
    theme_bw() +
    labs(
      title = paste0("PRCC (1000 spring simulations), daily % ", {{type}})) +
    xlab("PRCC (95% CI)") +
    ylab("Variable") +
  ...
  ggsave(paste0("./export/plots/spring", type, "_prcc",".bmp"), plot = plot_prcc)
}
