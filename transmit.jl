        ...
        elseif AnimalAgent.days_carrier != 0
         ...
         if length(possible_interactions) > 0
            for i in 1:length(possible_interactions)
                ...
                num_contacts = length(possible_interactions)
                status_agent = AnimalAgent.status
                transmission = transmit_status!(AnimalAgent, animalModel, interacting_agent, possible_interactions; susceptible = :S, inf_stat = :IS, to_stat = :ES, beta = AnimalAgent.βₛ*AnimalAgent.bactopop_is)
                transmission = transmit_status!(AnimalAgent, animalModel, interacting_agent, possible_interactions; susceptible = :S, inf_stat = :IR, to_stat = :ER, beta = AnimalAgent.βᵣ*AnimalAgent.bactopop_r)
